...
frappe.ui.form.on("Manifest", {
  ...
  refresh: function (frm) {
    ...
    if (frm.doc.docstatus == 1 && frm.doc.name && !frm.doc.cargo_registration) {
      frm.add_custom_button(
        __("Cargo Registration"),
        function () {
          frappe.call({
            method:
              "vsd_fleet_ms.vsd_fleet_ms.doctype.manifest.manifest.create_cargo_registration_from_manifest",
            args: {
              manifest_name: frm.doc.name,
            },
            callback: function (r) {
              if (r.message) {
                frm.set_value("cargo_registration", r.message.name);
                frm.save_or_update();
      ...
      if (frm.doc.transporter_type == "In House") {
        ...
        frm.add_custom_button(
          ...
          function () {
            frappe.call({
              ...
              method:
                "vsd_fleet_ms.vsd_fleet_ms.doctype.trips.trips.create_vehicle_trip_from_manifest",
              callback: function (r) {
                if (r.message) {
        ...
        args_array = {
          ...
          sub_contactor_truck_license_plate_no:
            frm.doc.sub_contactor_truck_license_plate_no,
        };
        frm.add_custom_button(
          __("Vehicle Trip"),
          function () {
      ...
      function (doc, cdt, cdn) {
        ...
        if (row.cargo_allocation == "Trailers") {
          ...
          if (frm.doc.transporter_type == "In House") {
            if (!frm.doc.truck) {
              frappe.msgprint(
                "Please select truck before Allocating cargo to Truck",
              );
...
function handle_Assign_Button_Click(args_array) {
  ...
  frappe.call({
    ...
    method:
      "vsd_fleet_ms.vsd_fleet_ms.doctype.manifest.manifest.add_to_existing_manifest",
    callback: function (r) {
      if (r.message) {
        var doc = frappe.model.sync(r.message)[0];
        frappe.set_route("Form", doc.doctype, doc.name);
