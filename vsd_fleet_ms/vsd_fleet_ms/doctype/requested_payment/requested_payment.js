...
frappe.ui.form.on("Requested Payment", {
  onload: function (frm) {
    ...
    var html =
      '<button style="background-color: green; color: #FFF;" class="btn btn-default btn-xs" onclick="cur_frm.cscript.approve_request(\'' +
      frm +
      "');\">Approve</button> ";
    html +=
      '<button style="background-color: red; color: #FFF;" class="btn btn-default btn-xs" onclick="cur_frm.cscript.reject_request(\'' +
      frm +
      "');\">Reject</button>";
  ...
  refresh: function (frm) {
    ...
    disburseFundsButton.$wrapper
      .find('[data-fieldname="disburse_funds"]')
      .hide();
    $('*[data-fieldname="requested_funds"]').find(".grid-remove-rows").hide();
    $('*[data-fieldname="requested_funds"]')
    ...
    cur_frm.get_field("request_total_amount").wrapper.innerHTML =
    ...
    if (cur_frm.doc.requested_funds.length > 0) {
      ...
      cur_frm.get_field("total_amount").wrapper.innerHTML =
        '<p class="text-muted small">Total Amount Approved</p><b>USD ' +
        total_approved_usd.toLocaleString() +
        " <br> TZS " +
        total_approved_tsh.toLocaleString() +
        "</b>";
      ...
      if (
        total_usd >= 0 &&
        total_tsh >= 0 &&
        total_usd == total_paid_usd &&
        total_tsh == total_paid_tsh &&
        frm.doc.payment_status != "Paid"
      ) {
        frm.set_value("payment_status", "Paid");
        frm.save_or_update();
      } else if (
        (total_paid_tsh < total_tsh || total_paid_usd < total_usd) &&
        frm.doc.payment_status != "Waiting Payment"
      ) {
        frm.set_value("payment_status", "Waiting Payment");
        frm.save_or_update();
      }

      cur_frm.get_field("account_approval_buttons").wrapper.innerHTML =
        '<p class="text-muted small">Total Amount Paid</p><b>USD ' +
        total_paid_usd.toLocaleString() +
        " <br> TZS " +
        total_paid_tsh.toLocaleString() +
        "</b>";
    }
    //Make payment button
  ...
  make_payment: function () {
    frappe.model.open_mapped_doc({
      method:
        "vsd_fleet_ms.vsd_fleet_ms.doctype.requested_payment.requested_payment.make_payment",
      frm: cur_frm,
    });
  },
  ...
  show_hide_sections: function (frm) {
    frm.toggle_display(
      ["request_total_amount", "html1"],
      frm.doc.requested_funds.length > 0,
    );
  ...
  disburse_funds: function (frm) {
    ...
    frappe.call({
      method:
        "vsd_fleet_ms.vsd_fleet_ms.doctype.requested_payment.requested_payment.disburse_funds_for_trip_requests",
      args: {
        requested_payment_name: frm.doc.name,
      },
      freeze: true,
...
cur_frm.cscript.disburse_funds = function () {
  cur_frm.events.disburse_funds(cur_frm);
};
frappe.ui.form.on("Requested Fund Details", {
  form_render(frm, cdt, cdn) {
    frm.fields_dict.requested_funds.grid.wrapper
...
frappe.ui.form.on("Requested Fund Accounts Table", {
  ...
  expense_type: function (frm, cdt, cdn) {
    frappe.call({
      method:
        "erpnext.hr.doctype.expense_claim.expense_claim.get_expense_claim_account",
      args: {
        ...
        if (r.message) {
          ...
          if (r.message.account) {
            frappe.model.set_value(
              ...
              frm.events.get_account_currency(frm, cdt, cdn, r.message.account),
            );
          }
        }
      },
  ...
  expense_account: function (frm, cdt, cdn) {
    if (locals[cdt][cdn].expense_account) {
      var expense_account_currency = frm.events.get_account_currency(
        frm,
        cdt,
        cdn,
        locals[cdt][cdn].expense_account,
      );
  ...
  payable_account: function (frm, cdt, cdn) {
    if (locals[cdt][cdn].payable_account) {
      var payable_account_currency = frm.events.get_account_currency(
        frm,
        cdt,
        cdn,
        locals[cdt][cdn].payable_account,
      );
    ...
    frappe.confirm(
      ...
      function () {
        $.each(selected["requested_funds"], function (index, value) {
          frappe.call({
            method:
              "vsd_fleet_ms.vsd_fleet_ms.doctype.requested_payment.requested_payment.recommend_request",
            freeze: true,
            args: {
    ...
    frappe.confirm(
      ...
      function () {
        $.each(selected["requested_funds"], function (index, value) {
          frappe.call({
            method:
              "vsd_fleet_ms.vsd_fleet_ms.doctype.requested_payment.requested_payment.recommend_against_request",
            freeze: true,
            args: {
              request_doctype: "Requested Fund Details",
              request_docname: value,
    ...
    frappe.confirm(
      ...
      function () {
        $.each(selected["requested_funds"], function (index, value) {
          frappe.call({
            method:
              "vsd_fleet_ms.vsd_fleet_ms.doctype.requested_payment.requested_payment.approve_request",
            freeze: true,
            args: {
              request_doctype: "Requested Fund Details",
              request_docname: value,
    ...
    frappe.confirm(
      ...
      function () {
        $.each(selected["requested_funds"], function (index, value) {
          frappe.call({
            method:
              "vsd_fleet_ms.vsd_fleet_ms.doctype.requested_payment.requested_payment.reject_request",
            freeze: true,
            args: {
              request_doctype: "Requested Fund Details",
              request_docname: value,
    ...
    frappe.confirm(
      ...
      function () {
        $.each(selected["accounts_approval"], function (index, value) {
          frappe.call({
            method:
              "vsd_fleet_ms.vsd_fleet_ms.doctype.requested_payment.requested_payment.accounts_approval",
            freeze: true,
            args: {
              ...
              reference:
                locals["Requested Fund Accounts Table"][value].reference,
              user: frappe.user.full_name(),
            },
            callback: function (data) {
    ...
    frappe.confirm(
      ...
      function () {
        $.each(selected["accounts_approval"], function (index, value) {
          ...
          frappe.call({
            method:
              "vsd_fleet_ms.vsd_fleet_ms.doctype.requested_payment.requested_payment.accounts_cancel",
            freeze: true,
            args: {
              request_doctype: "Requested Fund Accounts Table",
              request_docname: value,
...
cur_frm.cscript.populate_child = function (
  reference_doctype,
  reference_docname,
) {
  frappe.model.with_doc(reference_doctype, reference_docname, function () {
    ...
    if (request_total_amount_tsh != 0 || request_total_amount_usd != 0) {
      ...
      cur_frm.get_field("request_total_amount").$wrapper[0].innerHTML =
        '<p class="text-muted small">Total Requested Amount</p><b>USD ' +
        request_total_amount_usd +
        " <br> TZS " +
        request_total_amount_tsh.toLocaleString() +
        "</b>";
